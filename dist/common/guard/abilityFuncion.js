...
function ProvidedGuard(reflectorParameter, authGrpcServiceParameter) {
  let AbilitiesGuard = class AbilitiesGuard {
    ...
    canActivate(context) {
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        const rules =
          this.reflector.get(
            abilities_decorator_1.CHECK_ABILITY,
            context.getHandler()
          ) || [];
        ...
        try {
          ...
          if (this.authGrpcService) {
            user = yield (0, rxjs_1.lastValueFrom)(
        ...
        try {
          ...
          try {
            for (
              var _e = true, rules_1 = __asyncValues(rules), rules_1_1;
              (rules_1_1 = yield rules_1.next()), (_a = rules_1_1.done), !_a;
              _e = true
            ) {
              _c = rules_1_1.value;
              _e = false;
              const rule = _c;
              let sub = {};
              ability_1.ForbiddenError.setDefaultMessage(
                (error) =>
                  `You are not allowed to ${error.action} on ${error.subjectType}`
              );
              // If rule does not exist in ability => throw error
              ability_1.ForbiddenError.from(ability).throwUnlessCan(
                rule.action,
                (0, ability_1.subject)(rule.subject, sub)
              );
            }
          } catch (e_1_1) {
            e_1 = { error: e_1_1 };
  ...
  AbilitiesGuard = __decorate([(0, common_1.Injectable)()], AbilitiesGuard);
